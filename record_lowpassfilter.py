...
def FFT_A():
    ...
    #t5 = time.time()
    #対数変換スペクトル作成
    spectrum_A_correction = np.abs(spectrum_A) + 1
    spectrum_A_log = np.log(spectrum_A_correction)
    #ケプストラム作成
    cepstrum_A = np.fft.fft(spectrum_A_log)
    #包絡線を作成
    cut = 180  #ローパスリフターのカットオフ指数
    cepstrum_A[cut:len(cepstrum_A) - cut] = 0  #ケプストラム波形の高次を0にする（低次のみを残す、ローパスリフター化）
    cepstrum_low_A = np.fft.ifft(cepstrum_A)
    #逆フーリエ変換（時系列データに戻す）
    #F_ifft_A = np.fft.ifft(spectrum_A)
    #F_ifft_real_A = F_ifft_A.real  #realは実部。ここでは実部のみを読み込むコマンド。
    ...
    #ケプストラム波形
    #x_axis = np.arange(0,len(cepstrum_A))/fs
    #plt.subplot(2, 1, 2)
    #plt.plot(x_axis, spectrum_A_log))
    #plt.xlim(0, 0.5)
    #plt.grid(which="both")
    #plt.yscale("log")
    ...
    #対数変換スペクトル
    #plt.subplot(2, 1, 2)
    #plt.plot(frequency_A, spectrum_A_log)
    #plt.xlim(0, fs/2)
    #plt.grid(which="both")
    #plt.yscale("log")
    #plt.xlabel("freqency(Hz)", fontsize=9)
    #plt.ylabel("log Amplitude Spectrum", fontsize=9)

    plt.savefig('/home/pi/Documents/admp441_data/'+filename_A+'.png')
    plt.close()
    #print('/home/pi/Documents/admp441_data/'+filename_A+'.png', 'saved')
    np.savetxt('/home/pi/Documents/admp441_data/'+filename_A+'spectrum', np.abs(spectrum_A), delimiter = " ", fmt='%.4f')
    np.savetxt('/home/pi/Documents/admp441_data/'+filename_A+'frequency', frequency_A, delimiter = " ", fmt='%.2f')
    np.savetxt('/home/pi/Documents/admp441_data/'+filename_A+'spectrum envelope', np.abs(cepstrum_low_A), delimiter = " ", fmt='%.5f')
...
def FFT_B():
    ...
    np.savetxt('/home/pi/Documents/admp441_data/'+filename_B+'spectrum', np.abs(spectrum_B), delimiter = " ", fmt='%.4f')
